...
import hypellm.recipes


@pytest.fixture
def mock_data():
    return [
        Datum(inputs="What is 2+2?", outputs="4"),
        Datum(inputs="What is the capital of France?", outputs="Paris"),
    ]
...
async def test_reasoned(mock_data):
    results = await hypellm.recipes.reasoned(mock_data)
    assert len(results) == len(mock_data)
    assert all(isinstance(r, Datum) for r in results)
...
def test_reasoned_sync(mock_data):
    results = hypellm.recipes.reasoned_sync(mock_data)
    assert len(results) == len(mock_data)
    assert all(isinstance(r, Datum) for r in results)
    assert all(hasattr(r, "reasoning") for r in results)
    assert all(r.reasoning == ["step 1", "step 2", "step 3"] for r in results)
...
async def test_inverted(mock_data):
    results = await hypellm.recipes.inverted(mock_data)
    assert len(results) == len(mock_data)
    assert all(
...
def test_inverted_sync(mock_data):
    results = hypellm.recipes.inverted_sync(mock_data)
    assert len(results) == len(mock_data)
    assert all(
        r.inputs == mock.outputs and r.outputs == mock.inputs for r, mock in zip(results, mock_data)
    )
